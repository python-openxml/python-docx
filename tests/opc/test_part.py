...
class DescribePart(object):
    """Unit-test suite for `docx.opc.part.Part` object."""
...
class DescribePartRelationshipManagementInterface(object):
    ...
    def docx_rel(self, request, rtype, doc_part, rId_, url_):
        rel_ = instance_mock(request, _Relationship, rId=rId_, target_ref=url_)
        rel_.reltype = rtype
    ...
    def rel_types_fixture(self, request):
        ...
        rels_ = dict(("rId%d" % (idx + 1), relationships_[idx]) for idx in range(3))
        expected_parts = set(
