...
def given_a_paragraph_having_style(context, style_state):
    paragraph_idx = {"no specified": 0, "a missing": 1, "Heading 1": 2, "Body Text": 3}[
        style_state
    ]
...
def when_I_assign_a_style_type_to_paragraph_style(context, style_type):
    ...
    style_spec = {"style object": style, "style name": "Heading 1"}[style_type]
    paragraph.style = style_spec


@when('I assign bookmark = paragraph.start_bookmark("Target")')
def when_I_assign_bookmark_eq_paragraph_start_bookmark(context):
    paragraph = context.paragraph
    context.bookmark = paragraph.start_bookmark("Target")
...
@when("I end bookmark by calling paragraph.end_bookmark(bookmark)")
def when_I_end_bookmark_by_calling_paragraph_end_bookmark_with_bookmark(context):
    paragraph = context.paragraph
    paragraph.end_bookmark(context.bookmark)
