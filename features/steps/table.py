...
def given_a_Cell_object_with_vertical_alignment_as_cell(context, state):
    table_idx = {"inherited": 0, "bottom": 1, "center": 2, "top": 3}[state]
...
def given_a_table_having_a_width_of_width_desc(context, width_desc):
    col_idx = {"no explicit setting": 0, "1440": 1}[width_desc]
...
def given_a_table_having_alignment_alignment(context, alignment):
    table_idx = {"inherited": 3, "left": 4, "right": 5, "center": 6}[alignment]
...
def given_a_table_having_an_autofit_layout_of_autofit(context, autofit):
    tbl_idx = {"no explicit setting": 0, "autofit": 1, "fixed": 2}[autofit]
    document = Document(test_docx("tbl-props"))
...
def given_a_table_having_style(context, style):
    table_idx = {"no explicit": 0, "Table Grid": 1, "Light Shading - Accent 1": 2}[
        style
    ]
...
def when_I_assign_bookmark_eq_cell_start_bookmark(context):
    cell = context.cell
    context.bookmark = cell.start_bookmark("Target")
